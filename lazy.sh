...
then
	echo -e "$GREEN{-} Começando o scan ativo $ENDCOLOR"
        amass enum -active -brute -d $1 -w ~/wordlists/dns/smallDns.txt -rf /home/carrazza/resolvers.txt >>   ${passive}
...
cat ${finalfile} | xargs -P 15 -I {} bash -c "waybackurls {} | grep $2 | unfurl -u domain | anew wayback"
...
cat ${finalfile} | httprobe | tee ${pasta}probed 
...
cat $DnsFile | aquatone -out aquatone
